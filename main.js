document.addEventListener("DOMContentLoaded", async () => {
  ...
  function getImageColors(imgElement) {
    return new Promise((resolve) => {
      ...
      for (let i = 0; i < imageData.length; i += 4) {
        ...
        if (
          brightness <
          (darkest[0] * 299 + darkest[1] * 587 + darkest[2] * 114) / 1000
        ) {
          darkest = [r, g, b];
        }
        ...
        if (
          brightness >
          (lightest[0] * 299 + lightest[1] * 587 + lightest[2] * 114) / 1000
        ) {
          lightest = [r, g, b];
        }
  ...
  function updateAdaptiveTheme(colorData) {
    ...
    const cssVariables = {
      ...
      "--bg-secondary": `linear-gradient(135deg, ${toHex(
        sidebarPrimary
      )} 0%, ${toHex(sidebarSecondary)} 100%)`,
      "--bg-tertiary": `linear-gradient(135deg, ${toHex(
        sidebarSecondary
      )} 0%, ${toHex(adjustColorBrightness(sidebarSecondary))} 100%)`,
  ...
  let favorites = new Set(
    JSON.parse(localStorage.getItem("buddy-music-favorites") || "[]")
  );

  function saveFavorites() {
    localStorage.setItem(
      "buddy-music-favorites",
  ...
  function getSettings() {
    return (
      JSON.parse(localStorage.getItem("buddy-music-settings")) || {
        fontSize: "medium",
        fontFamily: "inter",
  ...
  function updateQueueView() {
    ...
    if (queue.length === 0) {
      ...
      emptyMessage.textContent =
        "Queue is empty. Right-click songs to add them to queue.";
      elements.queueList.appendChild(emptyMessage);
      return;
    }

    queue.forEach((song, index) => {
      const div = createPlaylistItem(song, index, () => {
        const songIndex = songs.indexOf(song);
        loadSong(songIndex);
  ...
  async function loadSong(index) {
    ...
    if (songId !== currentlyPlayingSongId) {
      ...
      if (
        document.documentElement.getAttribute("data-theme") === "adaptive" &&
        metadata.coverUrl
      ) {
        const img = new Image();
        img.crossOrigin = "Anonymous";
    ...
    if (elements.currentSongTitle)
      elements.currentSongTitle.textContent = metadata.title;
  ...
  function startVisualization() {
    ...
    function updateColors() {
      ...
      if (!isPlaying) {
        ...
        currentColors = {
          ...
          darkest: interpolateColors(
            currentColors.darkest,
            baseColors.darkest,
            fadingSpeed
          ),
          lightest: baseColors.lightest,
          mostColorful: interpolateColors(
            currentColors.mostColorful,
            baseColors.mostColorful,
            fadingSpeed
          ),
        };
      ...
      const bassIntensity =
        dataArray.slice(0, 4).reduce((a, b) => a + b, 0) / 4;

      const targetIntensity = Math.min(average / 128, 1) * 0.3;
      const targetBassBoost = Math.min(bassIntensity / 200, 1) * 0.2;

      prevIntensity =
        prevIntensity + (targetIntensity - prevIntensity) * smoothingFactor;
      prevBassBoost =
        prevBassBoost + (targetBassBoost - prevBassBoost) * smoothingFactor;
      ...
      currentColors = {
        ...
        mostColorful: interpolateColors(
          currentColors.mostColorful,
          baseColors.mostColorful.map((c) =>
            Math.min(255, c + prevIntensity * 40)
          )
        ),
      };
    ...
    function isColorsSimilar(colors1, colors2, threshold = 2) {
      return ["primary", "secondary", "darkest", "mostColorful"].every(
        (key) => {
          return colors1[key].every(
            (c, i) => Math.abs(c - colors2[key][i]) < threshold
          );
        }
      );
    }
  ...
  function playNext() {
    ...
    if (queue.length > 0) {
      ...
      const currentShuffledIndex = shuffledSongs.indexOf(
  ...
  elements.shuffleBtn.addEventListener("click", () => {
    ...
    const iconSpan = elements.shuffleBtn.querySelector(
    ...
    if (isShuffled) {
      ...
      console.log(
        "Shuffled song order:",
        shuffledSongs.map((song) => song.metadata.title)
      );
    }
  });
  ...
  elements.audioPlayer.addEventListener("timeupdate", () => {
    const progress =
      (elements.audioPlayer.currentTime / elements.audioPlayer.duration) * 100;
    elements.progressBar.value = progress;
    elements.currentTimeSpan.textContent = formatTime(
      elements.audioPlayer.currentTime
    );
  });

  elements.audioPlayer.addEventListener("loadedmetadata", () => {
    elements.durationSpan.textContent = formatTime(
      elements.audioPlayer.duration
    );
  });
  ...
  function fadeOutAudio() {
    ...
    fadeOutInterval = setInterval(() => {
      if (elements.audioPlayer.volume > 0.05) {
        elements.audioPlayer.volume = Math.max(
          0,
  ...
  function fadeInAudio() {
    ...
    fadeInInterval = setInterval(() => {
      if (elements.audioPlayer.volume < 0.9) {
        elements.audioPlayer.volume = Math.min(
          1,
          elements.audioPlayer.volume + 0.1
        );
      } else {
  ...
  elements.progressBar.addEventListener("mousemove", (e) => {
    if (e.buttons === 1) {
      ...
      elements.audioPlayer.currentTime =
        percentage * elements.audioPlayer.duration;
    }
  });

  elements.coverArt.addEventListener("click", () => {
    elements.coverArt
      .closest(".album-art-wrapper")
  ...
  elements.themeSelect.addEventListener("change", (e) => {
    ...
    if (
      newTheme === "adaptive" &&
      songs[currentSongIndex]?.metadata?.coverUrl
    ) {
      const img = new Image();
      img.crossOrigin = "Anonymous";
      img.onload = async () => {
  ...
  function sortPlaylist(criterion) {
    ...
    sortedSongs.sort((a, b) => {
      switch (criterion) {
        ...
        case "artist":
          return (a.metadata.artist || "").localeCompare(
            b.metadata.artist || ""
          );
        case "album":
  ...
  function filterPlaylist(searchTerm) {
    ...
    if (decadeMatch) {
      ...
      filtered = songs.filter((song) => {
        ...
        return (
          songYear >= window.currentSearch.startYear &&
          songYear < window.currentSearch.endYear
        );
      });
    } else if (
      searchLower === "fav" ||
      searchLower === "favorite" ||
      searchLower === "favorites"
    ) {
      ...
      filtered = songs.filter((song) => {
        ...
        const artistAbbreviations = generateArtistAbbreviations(
          song.metadata.artist
        );
  ...
  function showAddAllToQueue(filteredSongs) {
    ...
    if (!addAllBtn) {
      ...
      addAllBtn.addEventListener("click", () => {
        const songsToAdd = tempSearchResults.filter(
          (song) => !queue.includes(song)
        );
        if (songsToAdd.length > 0) {
          ...
          addAllBtn.innerHTML =
            '<span class="material-symbols-rounded">check</span> Added to queue!';
          setTimeout(() => {
            addAllBtn.innerHTML = `
                            <span class="material-symbols-rounded">queue</span>
                            Add all to queue (${filteredSongs.length} songs)
  ...
  function updateQueueView() {
    elements.queueList.innerHTML = "";
    queue.forEach((song, index) => {
      const div = createPlaylistItem(song, index, (idx) => {
        songs = queue;
  ...
  function updatePlaylistView(songList) {
    ...
    songList.forEach((song, index) => {
      const div = createPlaylistItem(song, songs.indexOf(song), loadSong);
  ...
  document.addEventListener("keydown", (e) => {
    ...
    switch (e.code) {
      ...
      case "ArrowRight":
        ...
        if (
          elements.audioPlayer.currentTime <=
          elements.audioPlayer.duration - 10
        ) {
          elements.audioPlayer.currentTime += 10;
        } else {
  ...
  elements.clearDatabaseBtn?.addEventListener("click", async () => {
    if (
      confirm(
        "Are you sure you want to clear the local database? This will remove all stored songs."
      )
    ) {
      try {
  ...
  async function clearEverything() {
    if (
      confirm(
        "Are you sure you want to delete ALL data? This includes songs, favorites, and all settings. This cannot be undone."
      )
    ) {
      try {
  ...
  if (songId !== currentlyPlayingSongId) {
    ...
    if ("mediaSession" in navigator) {
      ...
      navigator.mediaSession.setActionHandler("seekmode", () => {
        ...
        elements.loopBtn.style.color =
          loopState !== "none" ? "#1db954" : "#fff";
